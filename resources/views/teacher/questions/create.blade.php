...
@section('content')
        ...
        <div class="exam-summary-row">
            ...
            @else
                <div class="exam-summary-category">
                    ...
                    <a href="{{ route('teacher.question-categories.index') }}" class="btn btn-sm btn-primary">
    ...
    @if ($categories->isNotEmpty())
        <section class="content-panel add-item-panel" id="add-question-section">
            ...
            <div class="collapse {{ $openAddQuestion ? 'show' : '' }}" id="addQuestionForm" data-bs-parent="#add-question-section">
                <div class="add-item-inline-form mt-4">
                    @include('questions.partials.multi-form', [
                        'prefix' => 'teacher',
                        'formKey' => 'main',
                        'action' => route('teacher.questions.store', $exam),
            ...
            <div class="collapse {{ $openAddSummary ? 'show' : '' }}" id="addSummaryForm" data-bs-parent="#add-question-section">
                <div class="add-item-inline-form mt-4">
                    <form method="POST" action="{{ route('teacher.passage-groups.store', $exam) }}" class="admin-form">
                        @csrf
                        <div class="mb-3">
    ...
    @include('exams.partials.questions-panel-header', ['exam' => $exam, 'prefix' => 'teacher'])

    @include('exams.partials.questions-panel', ['exam' => $exam, 'prefix' => 'teacher', 'categories' => $categories])

    @if ($categories->isNotEmpty())
        @php($hasQuestions = $exam->questions->count() > 0)
        <section class="content-panel submit-panel">
            @if ($hasQuestions)
                <a href="{{ route('teacher.exams.show', $exam) }}" class="btn btn-success btn-lg">

<form method="POST" action="{{ $action }}" class="admin-form" data-guardian-picker-root data-guardian-search-url="{{ route('admin.guardians.search') }}">
    @csrf
    @isset($drawerId)
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <select id="class_id" name="class_id" class="form-select form-control{{ $useOldInput && $errors->has('class_id') ? ' is-invalid' : '' }}" required data-class-select data-selected-class="{{ $useOldInput ? old('class_id', $student->class_id) : $student->class_id }}">
                ...
                @foreach (($classes ?? (($selectedBranch ?? null) ? \App\Models\SchoolClass::visibleToBranch($selectedBranch->id)->orderBy('name')->get() : collect())) as $class)

...
@section('content')
    ...
    <section class="content-panel exam-summary-bar">
        ...
        @elseif ($exam->question_category_id)
            <div class="collapse mt-3" id="changeCategoryForm">
                <form method="POST" action="{{ route('branch.exams.category.update', $exam) }}" class="row g-2 align-items-end">
                    @csrf
                    @method('PUT')
        ...
        @else
            <form method="POST" action="{{ route('branch.exams.category.update', $exam) }}" class="row g-2 align-items-end mt-3">
                @csrf
                @method('PUT')
                <div class="col-sm-8 col-md-6">
    ...
    @include('exams.partials.questions-panel', ['exam' => $exam, 'prefix' => 'branch'])
